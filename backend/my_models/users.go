...
var UserWhere = struct {
	...
	Isadmin:   whereHelpernull_Bool{field: "\"users\".\"isadmin\""},
}
...
var (
	userAllColumns            = []string{"id", "email", "password", "created_at", "isadmin"}
	userColumnsWithoutDefault = []string{"email", "password"}
	userColumnsWithDefault    = []string{"id", "created_at", "isadmin"}
...
func (userL) LoadOwnerDocuments(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`documents`),
		qm.WhereIn(`documents.owner_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
